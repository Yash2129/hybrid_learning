...
class _FacultyProfileViewState extends State<FacultyProfileView> {
  ...
  Widget build(BuildContext context) {
    ...
    final r = FirebaseDatabase.instance.ref('Profiles').child(uid).child('FacultyName');
    r.once().then((DatabaseEvent event){
      name = event.snapshot.value.toString();
    });
    final r3 = FirebaseDatabase.instance.ref('Profiles').child(uid).child('Branch');
    r3.once().then((DatabaseEvent event){
      branch = event.snapshot.value.toString();
    });
    final r4 = FirebaseDatabase.instance.ref('Profiles').child(uid).child('CollegeId');
